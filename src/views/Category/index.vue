...
export default {
  ...
  created() {
      ...
      methods: {
          async init () {
            await this.$API.category.getCategoryPage({'page': this.page, 'pageSize': this.pageSize}).then(res => {
          ...
          deleteHandle(id) {
            ...
            this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
              ...
              this.$API.category.deleCategory(id).then(res => {
                if (res.code === 200) {
          ...
          submitForm(st) {
              ...
              if (this.action === 'add') {
                if (valid) {
                  ...
                  if (reg.test(classData.sort)) {
                    this.$API.category.addCategory({'name': classData.name,'type':this.type, sort: classData.sort}).then(res => {
                      console.log(res)
                      if (res.code === 200) {
              ...
              if (reg.test(this.classData.sort)) {
                this.$API.category.editCategory({'id':this.classData.id,'name': this.classData.name, sort: this.classData.sort}).then(res => {
                  if (res.code === 200) {
